...
def register_view(request):
    ...
    if request.method == 'POST':
        ...
        if password_data != password2_data:
            ...
            CompanyProfile.objects.create(user=user, company_name="", classification="other", country="")
...
def activate_account_view(request):
    if request.method == 'POST':
        ...
        try:
            subscription = Subscription.objects.get(user__username=username, serial_number=serial)
...
def manage_users_view(request):
    if not request.user.groups.filter(name='مدراء النظام').exists(): raise PermissionDenied
...
def edit_user_permissions_view(request, user_id):
    ...
    context = {'user_to_edit': user_to_edit, 'top_level_pages': top_level_pages}
